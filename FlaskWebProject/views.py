"""
Routes and views for the Flask application with Microsoft Authentication.
"""
...
def authorized():
    ...
    if "code" in request.args:
        ...
        # Use Azure AD username as local username
        username = session["user"].get("preferred_username")  # usually user email
        user = User.query.filter_by(username=username).first()
        if not user:
            # First-time login: create user in DB
            user = User(username=username)
            user.set_password(uuid.uuid4().hex[:10])  # temp random password
            db.session.add(user)
            db.session.commit()
...
def _build_auth_url(authority=None, scopes=None, state=None):
    return _build_msal_app(authority).get_authorization_request_url(
        scopes or [],
        state=state,
        redirect_uri=url_for("authorized", _external=True)
    )
